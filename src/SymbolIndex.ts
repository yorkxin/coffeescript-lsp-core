...
export class SymbolIndex {
  ...
  public indexFile(uriOrPath: Uri | string): Promise<void> {
  ...
  public async removeFile(uriOrPath: Uri | string) {
    const { locationURI } = this._normalizeAsURI(uriOrPath);
    return this._removeSymbolsOfFile(locationURI);
  }

  public find(query: string): Promise<SymbolInformation[]> {
    const queryInLowercase = query.toLowerCase();
    const dbQuery = {
      $where(this: ISymbolInformationEntry) {
    ...
    return new Promise((resolve, reject) => {
      this.db.loadDatabase(() => {
        ...
        this.db.find(dbQuery)
          .exec((err: Error, docs) => {
            if (err) { reject(err); }
            const entries = docs as ISymbolInformationEntry[];
  ...
  public _normalizeAsURI(uriOrPath: Uri | string | any): { fsPathWithoutScheme: string, locationURI: string } {
    let uri: Uri;
    ...
    const locationURI = uri.toString(true /* do not URIencode */);
  ...
  public _saveSymbols(symbols: SymbolInformation[], fsPath: string): Promise<void> {
  ...
  public _removeSymbolsOfFile(fsPath: string): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  public _serializeSymbol(symbolInformation: SymbolInformation): ISymbolInformationEntry {
